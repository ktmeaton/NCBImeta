    ...
    for ID in record['IdList']:
        ...
        for column in table_columns:
            ...
            for row in flatten_record_dict:
                ...
                if type(column) == str and column in row:
                    column_value = row[-1]
                    break

                # For complex column types, as list       
                elif type(column) == list:
                    print(column, column[column_index])
                    while column[column_index] in row:
                        print(row)
                        if column_index + 1 == len(column):
                            column_value = row[-1]    
                        column_index += 1
                    # Set column name to first element
                    column = column[0]
            ...
            for row in flatten_record_dict:
                ...
                for child in root.childNodes:
                    for child in child.childNodes:
                        if child.firstChild and "category" in child.attributes.keys(): 
                            if column == child.attributes["category"].value:
                                column_value = child.firstChild.nodeValue
                                break
                    break

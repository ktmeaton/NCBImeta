...
def test_xml_search_nodetext():
    ...
    NCBImetaUtilities.xml_search(
        test_root, test_search_list, test_current_tag, test_column_name, test_xml_dict
    )
    # assert 0
...
def test_HTTPErrorCatch(tmpdir):
    ...
    try:
        Entrez.read(ID_handle, validate=False)
        assert 1
    except RuntimeError:
...
def test_adv_xml_search_rel():
    ...
    test_xml_root = etree.fromstring(test_xml)
    test_payload = (
        "XPATH, //GBQualifier[GBQualifier_name/text() = 'organism']/GBQualifier_value"
    )
    test_xpath = test_payload.split(", ")[1]
    test_column_name = "GBOrganismName"
    test_xml_dict = {test_column_name: []}
...
def test_adv_xml_search_attr():
    ...
    test_xml = """
    <Links>
      <Link type="url" label="GEO Sample GSM3995467">
          https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM3995467
      </Link>
      <Link type="entrez" target="bioproject" label="PRJNA558013">
          558013
      </Link>
    </Links>
    """
...
def test_adv_xml_search_multibad():
    ...
    test_xml_dict = {test_column_name: []}
    with pytest.raises(NCBImetaErrors.ErrorXPathQueryMultiElement):
        NCBImetaUtilities.adv_xml_search(
            test_xml_root, test_xpath, test_column_name, test_xml_dict
        )

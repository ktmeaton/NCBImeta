...
def test_ncbimeta_OutputDirNotExists(tmpdir):
    ...
    try:
        ...
        # If an error is raised, it's Error Class is in the output (if add "as e")
        # print(str(e.output))
        assert 0
    # Cleanup
...
def test_ncbimeta_improper_table_name(tmpdir):
    ...
    with open(src_config, "r") as file:
        filedata = file.read()
    # Replace the target data
    filedata = filedata.replace("- Assembly", "- Assembly()")
    # Write to the destination file
    with open(config_file, "a") as file:
        file.write(filedata)
...
def test_ncbimeta_improper_column_name(tmpdir):
    ...
    with open(src_config, "r") as file:
        filedata = file.read()
    # Replace the target data
    filedata = filedata.replace("- AssemblyAccession", "- AssemblyAccession()")
    # Write to the destination file
    with open(config_file, "a") as file:
        file.write(filedata)
...
def test_ncbimeta_missing_xpath_query(tmpdir):
    """Test for successfully catching a missing XPath query"""
    tmpdir = tmpdir.strpath
    # Prep files and directories
    src_config = os.path.join(os.path.dirname(os.path.abspath(__file__)), "test.yaml")
    config_file = os.path.join(tmpdir, "tmp_config.yaml")
    ...
    filedata = filedata.replace(
        "XPATH, //Links/Link[@target='bioproject']/@label", "XPATH"
    )
    # Write to the destination file
    with open(config_file, "a") as file:
        file.write(filedata)
    ...
    try:
        ...
        # If an error is raised, it's Error Class is in the output
        assert "ErrorXPathQueryMissing" in str(e.output)
    # Cleanup
    os.remove(config_file)
